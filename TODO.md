...
- [x] **5.1** Обработка ошибок: Chrome не установлен, страница без текста,
  совпадений нет, сервис выключен.
  > Сделано: ChromeLauncher (FAILED→ERROR+тост), NO_MATCH при пустом результате,
  > баннер при выключенной службе, валидация URL/запроса в onRun.
- [x] **5.2** Логирование (`Log.d` с тегом) ключевых шагов для отладки.
  > Сделано: теги BrowserAutomation и ChromeLauncher (см. logcat в INSTALL.md).
- [x] **5.3** Инструкция `INSTALL.md`: как собрать APK, установить, включить
  спец. возможности и пользоваться.
  > Сделано: INSTALL.md (сборка, установка, включение службы, использование, FAQ).
- [x] **5.4** Финальная проверка: пройтись по сценарию из README, обновить
  README если что-то поменялось.
  > Сделано: README дополнен ссылкой на INSTALL.md, код пройден по сценарию.
  > Остаётся проверка сборки на устройстве/в Android Studio (нет SDK в контейнере).
